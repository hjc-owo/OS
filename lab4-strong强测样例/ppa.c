...
volatile u_int *timer = (u_int *) 0x100000;
...
void srv(u_int pvt) {
    u_int me = env->env_id;
    writef("%x: srv up, pvt is %x\n", me, pvt);

    sleep(1);

    int i;
    u_int *buf = (u_int *) 0x60000000;
    syscall_mem_alloc(0, (u_int) buf, PTE_V | PTE_R);
    for (i = 0; i < (tot >> 1); ++i) {
        u_int who = 0;
        u_int v = ipc_recv(&who, 0, 0);
        writef("%x: got %x from %x\n", me, v, who);
        uassert(v == who + me);
        buf[i << 1] = who;
        buf[i << 1 | 1] = v;
    }
...
void cli(u_int pvt, u_int *srvs) {
    u_int me = env->env_id;
    writef("%x: cli up, pvt is %x, pvt2 is %x\n", me, pvt, *srvs);
    int i;

    for (i = 0; i < (tot >> 1); ++i) {
        u_int dst = srvs[i];
        writef("%x: sending %x to %x\n", me, dst + me, dst);
...
void umain() {
    syscall_mem_alloc(0, (u_int) timer, PTE_V | PTE_R | PTE_LIBRARY);
    *timer = 0;
    if (fork_n(2))
        sheep();
    ...
    syscall_mem_alloc(0, (u_int) shm, PTE_V | PTE_R | PTE_LIBRARY);
